...
Обычный смысловой цикл EonMemory **интерактивный** и использует последовательные вызовы `input()`. Поэтому открой интерактивную терминальную сессию с подключённым stdin/PTY и только внутри неё запусти без параметров:

```bash
python /mnt/data/EonMemory.py
```

Не запускай эту команду как одноразовый неинтерактивный вызов без передачи ввода: программа корректно остановится на приглашении и будет ждать ответ, а инструмент может ошибочно завершить сеанс по таймауту.

Последовательно передай в ту же интерактивную сессию:

1. рассуждение;
2. следующий вопрос;
3. изменение доступного регистра либо `N`.

Если доступный инструмент технически не поддерживает интерактивный PTY-сеанс, допустим только управляемый резерв: передать все три ответа через stdin в одном вызове и дождаться `Шаг сохранён: NN`. Сам по себе таймаут процесса, ожидающего `input()`, не является ошибкой EonMemory.
